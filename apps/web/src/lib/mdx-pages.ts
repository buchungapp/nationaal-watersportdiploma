...
export async function getAllDiplomalijnInstructeurPages() {
  ...
  const pageFilenames = await glob("**/page.{mdx,tsx}", {
  ...
  const pages = await Promise.all(
    pageFilenames.map(async (pageFilename) => {
      ...
      if (!imported.page) return null;

      // Strip either "/page.ext" (nested) or "page.ext" (root landing).
      const filePath = pageFilename.replace(/(?:^|\/)page\.(mdx|tsx)$/, "");

      // biome-ignore lint/style/noNonNullAssertion: intentional
      const slug = filePath === "" ? null : filePath.split("/").at(-1)!;

      return {
        pathSegments: filePath === "" ? [] : filePath.split("/").slice(0, -1),
        slug,
        ...imported.page,
      };
    }),

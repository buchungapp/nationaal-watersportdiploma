// Pure generator logic. No "use server", no Next.js imports. Used by the
// corpus eval harness (eval-runner.ts + experiment-prior-context*.ts)
// and kept as a standalone module so the eval pipeline continues to
// run after the portfolio-helper-sandbox UI was retired in favour of
// the leercoach. If we ever resurrect an interactive portfolio-helper
// surface, it imports from here.
...
/**
 * Kept exported for historical eval scripts that log this value into
 * reports for reproducibility. Re-exports CHAT_MODEL so the runtime
 * sandbox path always uses the production model; scripts that want
 * a pinned version should hardcode their own local constant.
 */
...
export async function runQuestionGeneration(
  tree: RubricTree,
  // Optional override for eval scripts pinning a specific model id.
  // Runtime callers omit and pick up CHAT_MODEL via MODEL_ID.

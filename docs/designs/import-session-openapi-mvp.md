...
Watersportdiploma/Nawadi. The current public API is generated from
`specifications/api.yaml` and served by `apps/api-server`. Buchung can consume
the OpenAPI contract with Hey API independently of the server implementation.
...
Build the N0/N1 contract on the existing OpenAPI-first API server:

- `PUT /location/{location-key}/cohort/{cohort-key}/import-session/{external-session-key}`
  creates or replaces a full vendor snapshot idempotently.
- `GET /location/{location-key}/import-session/{external-session-key}`
  retrieves import-session status by vendor key within a location.
- `GET /location/{location-key}/cohort/{cohort-key}/import-session`
...
Handlers stay thin in this PR and return a consistent `501 Not Implemented`.
Durable tables and core semantics can follow behind the contract:
`import_session`, `import_session_row`, `import_session_preview`, and
`import_session_row_commit`, with statuses `received`, `reviewing`,

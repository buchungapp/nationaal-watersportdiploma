...
export const remove = wrapCommand(
  "platform.media.remove",
  withZod(uuidSchema, z.void(), async (id) => {
    const query = useQuery();
...
export const listImages = wrapQuery(
  "platform.media.listImages",
...
export const createSignedUrl = wrapCommand(
  "platform.media.createSignedUrl",
  withZod(
    z.object({
      id: z.string().uuid(),
